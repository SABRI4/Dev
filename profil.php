...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($photo['error'] === UPLOAD_ERR_OK) {
        ...
        $query = "UPDATE users SET username = ?, email = ?, role = ?, photo = ? WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssdsi", $username, $email, $role, $photoPath, $user_id);
    } else {
        $query = "UPDATE users SET username = ?, email = ?, niveau = ? WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssdi", $username, $email, $role, $user_id);
    }
